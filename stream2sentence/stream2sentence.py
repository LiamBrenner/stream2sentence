...
def initialize_nltk():
    """Initialize NLTK by downloading required data for sentence tokenization."""
...
def initialize_stanza(language: str = "en"):
    """Initialize Stanza by downloading required data for sentence tokenization."""
...
def _remove_links(text: str) -> str:
    """Remove any links from the input text.
...
def _generate_characters(
    ...
    """Generate individual characters from a text generator.
...
async def _agenerate_characters(
    generator: AsyncIterator[str], log_characters: bool = False
) -> AsyncIterator[str]:
    """Generate individual characters from an async text generator.

    Args:
    ----
        generator (AsyncIterator[str]): Input text generator
        log_characters (bool): Whether to log the characters to the console
...
def _clean_text(
    ...
    """Clean the text by removing links and emojis.
...
async def _atokenize_sentences(
    text: str, tokenize_sentences=None
) -> AsyncIterator[str]:
    """Tokenize sentences from the input text.
    ...
    Yields:
    ------
        AsyncIterator[str]: An async iterator of sentences
    """
    if tokenize_sentences:
        sentences = tokenize_sentences(text)
    else:
...
def generate_sentences(
    ...
    """Generate well-formed sentences from a stream of characters or text chunks provided by an input generator.
...
async def agenerate_sentences(
    ...
    Args:
    ----
        generator (AsyncIterator[str]): An async generator that yields chunks of text as a stream of characters.
        context_size (int): The number of characters used to establish context for sentence boundary detection. A larger context improves the accuracy of detecting sentence boundaries. Default is 12 characters.
        minimum_sentence_length (int): The minimum number of characters a sentence must have. If a sentence is shorter, it will be concatenated with the following one, improving the overall readability. This parameter does not apply to the first sentence fragment, which is governed by `minimum_first_fragment_length`. Default is 10 characters.
        minimum_first_fragment_length (int): The minimum number of characters required for the first sentence fragment before yielding. Default is 10 characters.
        quick_yield_single_sentence_fragment (bool): If set to True, the generator will yield the first sentence fragment as quickly as possible. This is particularly useful for real-time applications such as speech synthesis.
    ...
    Yields:
    ------
        AsyncIterator[str]: An async iterator of complete sentences constructed from the input text stream. Each yielded sentence meets the specified minimum length requirements and is cleaned up if specified.
    ...
    async for char in _agenerate_characters(generator, log_characters):
        if char:
            buffer += char
            buffer = buffer.lstrip()
